...
class ChatModel with ChangeNotifier {
  ...
  // 添加缺失的showCmWindow方法
  Future<void> showCmWindow() async {
    // CM窗口显示逻辑
  }
  ...
  void showChatPage(MessageKey key) {
    changeCurrentKey(key);
  }
  
  void toggleCMFilePage() {
    // CM文件页面切换逻辑
  }
  
  void toggleCMChatPage(MessageKey key) {
    changeCurrentKey(key);
  }

  void showChatWindowOverlayComplete({Offset? chatInitPos}) {
    final overlayState = _blockableOverlayState.state;
    if (overlayState == null) return;
    if (isMobile &&
  ...
  send(ChatMessage message) {
    ...
    if (_currentKey.connId == clientModeID && parent.target != null) {
      bind.sessionSendChat(sessionId: sessionId.toString(), text: message.text);
    } else {
  ...
  void closeVoiceCall() {
    bind.sessionCloseVoiceCall(sessionId: sessionId.toString());
  }

  // 添加缺失的toggleChatOverlay方法
