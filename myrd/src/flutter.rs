...
pub mod connection_manager {
    ...
    impl FlutterHandler {
            ...
            V: Sized + serde::Serialize + Clone,
        {
            ...
            if let Some(s) = GLOBAL_EVENT_STREAM.try_read().unwrap().get(super::APP_TYPE_CM) {
...
pub fn push_global_event(channel: &str, event: String) -> Option<bool> {
    Some(GLOBAL_EVENT_STREAM.try_read().unwrap().get(channel)?.add(event))
}
...
pub fn start_global_event_stream(s: StreamSink<String>, app_type: String) -> ResultType<()> {
    ...
    let mut lock = GLOBAL_EVENT_STREAM.try_write().unwrap();
...
pub fn stop_global_event_stream(app_type: String) {
    let _ = GLOBAL_EVENT_STREAM.try_write().unwrap().remove(&app_type);
}
