...
# RustDesk Android 项目环境自动配置脚本 (CentOS/RHEL/Alibaba Cloud Linux)
# 版本: 2025-09-13
# 支持: CentOS 8+, RHEL 8+, Alibaba Cloud Linux 3
...
if ! java -version 2>&1 | grep -q "11\."; then
    ...
    # 查找 JAVA_HOME 路径
    JAVA_HOME=$(find /usr/lib/jvm -name "java-11-openjdk*" | head -1)
...
# 在 CentOS 上查找 libclang
LIBCLANG_PATH=$(find /usr -name "libclang.so*" 2>/dev/null | head -1 | xargs dirname 2>/dev/null || echo "/usr/lib64")
